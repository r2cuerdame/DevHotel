...
The MCP server exposes 37 tools over the same contract the app uses: create/start/sleep/clone/rename/delete a Room, run commands in it, apply and undo Quick Changes, read logs, retained command output, components and diagnostics, push and pull workspace files, reset a Room, request a guarded Host resync under the Room's revocable grant, and — for Android Rooms — build, install and launch an APK, take a screenshot of the phone, and attach, heartbeat and release a shared USB test device under an exclusive lease.
