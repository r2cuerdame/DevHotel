...
The MCP server exposes 44 tools over the same contract the app uses: create/start/sleep/clone/rename/delete a Room, run commands in it, apply and undo Quick Changes, read logs, retained command output, components and diagnostics, push and pull workspace files, reset a Room, request a guarded Host resync under the Room's revocable grant, and — for Android Rooms — build and install APKs, launch/stop tracked apps, wait for or tap app-scoped text, inspect a sanitized UI tree, read package-UID-scoped logs, run a bounded crash scenario, take a screenshot, and attach, heartbeat and release a shared USB test device under an exclusive lease.
