...
New Local Folder Rooms import Host source through a short-lived read-only mount into a Room-owned source volume. **Sync from Host** is one guarded inspect/refuse-or-confirm/reset/resync operation: meaningful Room drift is named before anything changes, confirmation is bound to the inspected snapshot, and a later edit aborts the staged import. Existing Local Folder Rooms stay in visible `legacy-host-bind` compatibility mode with Agent mutation/Clone blocked until the user selects **Move into Hotel**. Import is currently a full staged copy rather than incremental/COW sync. Android Clean Build snapshots the complete Room-owned source, uses the pinned image with disposable SDK/Gradle state, and exports APKs plus verified provenance under the Room; Web Build/Test and durable per-Job recovery still lack the shared immutable `StateRevision` implementation.
