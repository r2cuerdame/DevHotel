...
export default defineConfig(({ command }) => {
  const resolvedIdentity = resolveBuildIdentity(repoRoot, packageVersion)
  // A dev server can rebuild after this config is evaluated. Never let live
  // development evidence claim immutable source provenance.
  const buildIdentity = buildIdentityForViteCommand(resolvedIdentity, command)
