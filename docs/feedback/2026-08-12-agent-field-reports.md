...
- Long `run_in_room` output truncated by message limits → tool guidance: redirect to a file and fetch with `room_pull_file` (real streaming remained backlog item 3; **superseded 2026-08-25** — the exec contract itself is now bounded, filterable and retained, see item 3).
...
3. **Live output for long commands.** *Mostly shipped 2026-08-25.* `run_in_room` now returns a bounded, filterable window (`maxBytes`/`maxLines`, `head`/`tail`, `include`/`exclude`) and reports raw versus returned bytes and lines; whatever the response could not carry is retained under the Room and read back with `read_run_output`, which pages by byte offset and works **while the command is still running**. `list_room_runs` shows active runs with the bytes produced so far, which is the hung-versus-busy signal, and lets a caller pick a run back up after the control API connection drops.
   Still open: the call itself is synchronous, so a client that disconnects cannot re-attach to it or cancel it, and retention is Hotel storage rather than a durable Job — the resumable job handle stays goal.md §5.10 work.
