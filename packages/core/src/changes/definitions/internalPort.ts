...
export const internalPortChange: ChangeDefinition<{ port: number }> = {
  ...
  async preflight(ctx, p) {
    ...
    if (ctx.room().provider === 'android') {
      throw new Error('Android emulator screen relay uses the fixed internal port 6080')
    }
  ...
  async apply(ctx, p, steps) {
    ...
    if (ctx.isAwake()) {
      ...
      const { hostPort } = await ctx.backend.recreateAnchor({
        roomId: ctx.roomId,
        internalPort: p.port
      })
