...
export class FakeBackend implements IsolationBackend {
  ...
  async execInRoom(_roomId: string, _cmd: string[], opts?: ExecOpts): Promise<ExecResult> {
    if (!opts?.onStdout && !opts?.onStderr) return this.execResult
    const stdout = this.execChunks?.stdout ?? (this.execResult.stdout ? [this.execResult.stdout] : [])
    const stderr = this.execChunks?.stderr ?? (this.execResult.stderr ? [this.execResult.stderr] : [])
    for (const chunk of stdout) opts.onStdout?.(chunk)
    for (const chunk of stderr) opts.onStderr?.(chunk)
    return { code: this.execResult.code, stdout: '', stderr: '' }
  }
