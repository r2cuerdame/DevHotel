...
describe.skipIf(!process.env.DEVHOTEL_SMOKE)('Windows linked-folder to Linux Room drift', () => {
  ...
  beforeAll(async () => {
    ...
    write(source, '.devhotel-sync-include', 'app/build/generated/tracked.txt\r\n')
  ...
  it('ignores build-only changes, keeps an opted-in generated path, and reports only real source drift', async () => {
    ...
    const mutate = await runDocker([
      ...
      'sh',
      '-lc',
      "mkdir -p /workspace/.gradle/new /workspace/app/build/outputs/apk/debug; printf disposable > /workspace/.gradle/new/cache.bin; printf apk > /workspace/app/build/outputs/apk/debug/new.apk"
    ])
