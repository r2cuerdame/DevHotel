...
export function makeTools(getClient: () => Promise<ControlClient>): ToolDef[] {
  return [
    ...
    },
    {
      ...
      description:
        "Capture the Android room's phone screen into an immutable Room artifact and return its metadata plus a directly reviewable image. Give it an explicit portable .png filename. Default 'auto' uses sharp guest-side screencap; pass mode 'screen' to grab the emulator display instead — that also captures apps that set FLAG_SECURE. The room must be awake. Never point host screen-capture or host input automation at the DevHotel preview window.",
      schema: {
        roomId: zRoomId,
    ...
    },
    {
      ...
      description:
        'Export a screenshot artifact into a new safe repo-relative .png path in its Hotel-owned Room workspace. Never accepts Host paths or overwrites a project file; returns GitHub Markdown.',
      schema: {
        roomId: zRoomId,
