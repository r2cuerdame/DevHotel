...
export function makeTools(getClient: () => Promise<ControlClient>): ToolDef[] {
  return [
    ...
    },
    {
      name: 'run_in_room',
      description:
        'Run a command inside the room (never on the host). Use for installs, builds, scripts. Returns exit code, stdout, stderr. Output is buffered until exit — for long/verbose commands redirect to a file (`... > /workspace/out.log 2>&1`) and fetch it with room_pull_file so nothing is lost to message limits. UI input belongs here too: drive an Android room with `adb -s emulator-5554 shell input ...` against its own emulator — a leased physical phone goes through android_device_adb instead — never with host mouse/keyboard automation aimed at the DevHotel preview, which would take over the real desktop of whoever is running DevHotel.',
      schema: {
        roomId: zRoomId,
